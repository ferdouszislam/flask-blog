...
from blog_webapp.models import Post, User
...
def populate_posts_to_db(size=10):
    posts_to_populate = _posts[:min(size, len(_posts))]
    for post in posts_to_populate:
        # author = User.query.get(post['user_id'])
        # post = Post(title=post['title'], content=post['content'], author=author)
        post = Post(title=post['title'], content=post['content'], user_id=post['user_id'])

        # don't insert if post already exists in database
        existing_post = Post.query.filter_by(user_id=post['user_id']).first()
...
def get_db_entries(model):
    db_entries = []
    if model == 'users':
        db_entries = User.query.all()
    elif model == 'posts':
        db_entries = Post.query.all()
    return db_entries


if __name__ == "__main__":
    # populate_posts_to_db()
    # print(len(get_db_entries('posts')))
    print('populate_db.py ran successfully')
